...
static void clip_addchar(clip_workbuf *b, wchar_t chr, int attr, truecolour tc)
{
	...
	b->tcbuf = sresize(b->tcbuf, b->buflen * sizeof(truecolour), truecolour);
	b->tcptr = b->tcbuf + b->bufpos * sizeof(truecolour);
    }
    *b->textptr++ = chr;
    *b->attrptr++ = attr;
    *b->tcptr++ = tc;
...
static void clipme(Terminal *term, pos top, pos bottom, int rect, int desel)
{
    ...
    buf.tcptr = buf.tcbuf = snewn(buf.buflen * sizeof(truecolour), truecolour);
